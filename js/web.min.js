...
function showTop100(v, options) {
    ...
    if (options.buff == "none") {
        f = function(g) { return g.getMaxStatus(0); };
        ...
        f = function(g) { return g.getMaxStatus(1); };
        ...
        f = function(g) { return g.getMaxStatus(2); };
...
function insertOptions() {
    ...
    if (RESOURCES_LOADED["evaluation"]) {
        ...
        $("#options-evaluation").append("<p class='warning'>Type classification is not loaded. Please load it and refresh the webpage if you want to use the classification.</p>");
    }
    ...
    $("#clear_custom_recommends").click(function() {
        clearCustomRecommends();
    });
    $("#clear_card_comparison").click(function() {
        clearCardComparison();
    });
}
...
function saveOptions() {
    ...
    $("#options-resources input:checkbox").each(function() {
        var cname = $(this).attr('name');
        setCookie(cname, $(this).is(':checked'));
    });

    setCookie(COOKIE_LOAD_HETEROGENEOUS_TYPES, $("#options-heterogeneous-types").is(':checked'));
    ...
    if (RESOURCES_LOADED["evaluation"]) {
        ...
        setCookie(EVALUATION_COOKIE_NAME, evaluation_id, 180);
    }

    setCookie(COOKIE_TRANSLATE, $("#options-translate").is(":checked"));
}
...
function insertGuardian(gid, tid, custom_skills) {
      ...
      $("<table id='stats'></table>").addClass("stats")
        ...
        .append(
          $("<tr></tr>")
            ...
            .append($("<td></td>").html(g.getMinHP() + g.getMinMP() + g.getMinATK() + g.getMinDEF() + g.getMinAGI() + g.getMinWIS()))
        )
        .append(
          $("<tr></tr>")
            ...
            .append($("<td></td>").html(g.getStoneHP() + g.getStoneMP() + g.getStoneATK() + g.getStoneDEF() + g.getStoneAGI() + g.getStoneWIS()))
        )
        .append(
          $("<tr></tr>")
            ...
            .append($("<td></td>").html(g.getMaxHP() + g.getMaxMP() + g.getMaxATK() + g.getMaxDEF() + g.getMaxAGI() + g.getMaxWIS()))
        )
        .append(
          $("<tr></tr>")
            ...
            .append($("<td></td>").html(g.getMaxStonedHP() + g.getMaxStonedMP() + g.getMaxStonedATK() + g.getMaxStonedDEF() + g.getMaxStonedAGI() + g.getMaxStonedWIS()))
        )
    );
    ...
    .append(
        $("<div></div>")
            .append($("<a href='javascript:'><img src='images/save_24x24.jpg' alt='Save skills for this card of the selected type' title='Save skills for this card of the selected type'/></a>").click(function() {
                var custom_skills = new Array();
                $("#left option:selected").each(
                    function() {
                        var sid = $(this).val();
                        var skill = Skill.get(sid);
                        if (skill != null && $.inArray(skill, custom_skills) == -1)
                            custom_skills.push(sid);
                    }
                );
    ...
    $(left).append(
      $("<div id='options'></div>")
        ...
        .append(
          $("<label></label>")
            .append($("<input type='checkbox' id='opt-stoned'/>").attr('checked', cur_options.stoned))
            .append("&nbsp;Fully Stoned")
        )
		.append("<br/>")
        .append(
          $("<label></label>")
            .append($("<input type='checkbox' id='opt-enem-stoned'/>").attr('checked', cur_options.enemy_stoned))
            .append("&nbsp;Enemy Card Fully Stoned")
        )
    );
    ...
    $(left)
      .append($("<button>Calculate</button>").click((function(gid, tid) {
          return function() {
			  ...
			  cur_options.stoned = $("#opt-stoned").is(':checked');
			  cur_options.enemy_stoned = $("#opt-enem-stoned").is(':checked');
    ...
    $(left).append($("<div>Attribute</div>").addClass("header"))
    ...
    for (var k = 0; k < oneshots.length; k++) {
        ...
        /* Insert the header. */
        $(center).append(
        $("<div>" + unable.title + ": </div>")
            .addClass("header")
            .append("<span id='" + unable.id + "'></span>")
            .append(
            $("<a href='javascript:'><img class='qmark' width='16' src='images/question.png' /></a>")
...
/*
                .append($("<a href='javascript:' class='gclnk'></a>")
                    .append($("<span></span>").addClass("guardian-name").html(opponent.name))
                    .click((function (gid) {
                    return function() {insertGuardian(gid, 0, null);};
                    })(opponent.id))
                )
*/
        ...
        $("#" + unable.id).html(unable.ccount + " (" + unable.tcount + ")");
    }
...
<li><a class='internal' href='javascript:'>ATK</a> <a href='?" + locale.getURLArgs("top=atk&buff=recommended") + "' target='_blank'><img src='images/external-link.png' /></a></li> \
    ...
    $("#options-dialog").dialog({
        ...
        position: {my: "center", at: "top", of: window},
        width: 800,
        close: function(event, ui) { saveOptions(); }
    });
    insertOptions();
