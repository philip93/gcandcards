...
function loadResource(filename, filetype, callback) {
    ...
    if (filetype == "js") { // if filename is a external JavaScript file
        ...
        fileref.setAttribute("src", filename + "?nocache=" + (new Date()).getTime());
...
var Loader = (function() {
    var loadAll = function(all, callback) {
            ...
            loadResources(resources, function() {
                 ...
                 */
                if (goptions.isTranslationNeeded()) {
    ...
    return {
        load: function(all, callback) {
            /* Load all files. */
            if (typeof all == 'boolean')
                loadAll(all, callback);
            /* Load a single file. If JS_DEBUG is false, ".min" version will be loaded for Javascript files. */
                ...
                if (type != null)
                    loadResource(fn, type, callback);
            /* Load necessary and enabled files. */
            } else if (typeof all == 'function' && typeof callback == 'undefined')
                loadAll(false, all);
        }
    };
})();
