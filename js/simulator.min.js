...
function Simulator(my_party, oppo_party, options) {
    ...
    var applyBuffs = function(r, d, g1, g2) {
        ...
        List.iter(function(skill) {
            if (skill.isBuff() || skill.isDebuff())
                applyBuff(r, d, g1, g2, skill);
        }, g1.getSkills());
    };
    
    var applyHeal = function(r, d, g1, g2) {
        if (g1.battle.status.mp < Skill.heal.cost)
            return;

        g1.battle.status.mp -= Skill.heal.cost;
        g1.battle.status.hp += Math.floor(g1.status.hp * 0.55);
    ...
    var applyManaDrain = function(r, d, g1, g2) {
        if (g1.battle.status.mp < Skill.mana.cost)
            return;
    ...
    var applyFB = function(r, d, g1, g2) {
        if (g1.battle.fb && g1.battle.status.mp >= Skill.fb.cost) {
            g1.battle.fb = false;
    ...
    var applyMR = function(r, d, g1, g2) {
        var res = false;
        if (g1.battle.mr && g1.battle.status.mp >= Skill.mr.cost) {
            g1.battle.mr = false;
            g1.battle.status.mp -= Skill.mr.cost;
            r.append(d, g1, g2, g1.name + "'s " + Skill.mr.name, MSG_OTHER);
            if (Math.random() < options[d.attacker].mr) {
    ...
    var applyLS = function(r, d, g1, g2) {
        ...
        if (g1.battle.ls) {
            g1.battle.ls = false;
            if (g1.battle.status.mp > 0 && Math.random() < options[d.attacker].ls) {
    ...
    var applySap = function(r, d, g1, g2) {
        var res = false;
        if (g1.battle.sap && Skill.sap.cost <= g1.battle.status.mp) {
            g1.battle.sap = false;
            r.append(d, g1, g2, g1.name + "'s " + Skill.sap.name, MSG_BUFF);
            if (Math.random() < options[d.attacker].sap) {
    ...
    var applyEP = function(r, d, g1, g2) {
        var res = false;
        if (g1.battle.ep && Skill.ep.cost <= g1.battle.status.mp && 
    ...
    var applyDS = function(r, d, g1, g2) {
        ...
        if (g1.battle.ds && Skill.ds.cost <= g1.battle.status.mp && Math.random() < rate) {
    ...
    var applyRevival = function(r, d, g1, g2) {
        ...
        if (g1.battle.revival) {
            ...
            if (Skill.revival.cost <= g1.battle.status.mp && Math.random() < rate) {
    ...
    var applyDR = function(r, d, g1, g2, damage) {
        if (g1.battle.dr && damage > 0 && g1.battle.status.mp >= Skill.dr.cost) {
            g1.battle.status.mp -= Skill.dr.cost;
    ...
    var applyCurse = function (r, d, g1, g2, damage) {
        if (g1.battle.curse) {
            ...
            r.append(d, g1, g2, g2.name + "'s MP reduced by " + damage);
            return true;
        } else
            return false;
    };
    ...
    var getSkillDamage = function(d, g1, g2, skill) {
        ...
        if (skill == null) {
            ...
            var ex2 = d.getAttackerEx2();
            var exskill = ex2 == null ? null : ex2.getSkill();
            if (exskill != null && exskill.pcs(Skill.qs))
                buff1 += exskill.powup;
            ...
            buff2 = g2.battle.def;
            damage = Math.max(1, Calculator.getDamage(atk, sk, buff1, attr, def, buff2));
        } else if (skill == Skill.gs && Skill.gs.cost <= g1.battle.status.mp) {
            atk = g1.battle.status.atk;
            ...
            var ex2 = d.getAttackerEx2();
            var exskill = ex2 == null ? null : ex2.getSkill();
            if (exskill != null && exskill.scs(Skill.gs))
                rate += exskill.sucup;
            ...
            attr = 1;
            damage = Math.max(1, Calculator.getDamage(atk, sk, buff1, attr, def, buff2));
        } else if (skill == Skill.bg && Skill.bg.hp_cost < g1.battle.status.hp) {
            atk = g1.battle.status.atk;
            sk = 0.5;
            buff1 = g1.battle.atk;
            attr = 1;
    ...
    var getBestSkill = function(d, g1, g2) {
        ...
        if (g1.hasSkill(Skill.mana) && g1.battle.status.mp < g1.status.mp && g2.battle.status.mp > 0)
            return Skill.mana;

        if (g1.hasSkill(Skill.heal) && g1.battle.status.hp < g1.status.hp && g1.battle.status.mp >= Skill.heal.cost)
            return Skill.heal;

        if (g1.hasSkill(Skill.majorheal) && g1.battle.status.hp < g1.status.hp && g1.battle.status.mp >= Skill.heal.cost)
            return Skill.majorheal;

        if (g1.hasSkill(Skill.gs) && g1.battle.status.mp >= Skill.gs.cost)
            return Skill.gs;
        ...
        if (damage < g2.battle.status.hp) {
            ...
            if (g1.hasSkill(Skill.life) && g1.battle.status.hp < g1.status.hp)
                return Skill.life;
        }
    ...
    var doAttack = function(r, d, g1, g2, skill) {
        ...
        if (skill == Skill.heal)
            ...
            if (skill != null) {
                g1.battle.status.mp -= skill.cost;
                g1.battle.status.hp -= skill.hp_cost;
            }

            r.append(d, g1, g2, g1.name + "'s " + name, skill == Skill.qs ? MSG_QS : MSG_ATTACK);

            /* A physical attack is evaded. */
            if ((skill == null || skill.isPhysical()) && d.swap(function() { return applyDS(r, d, g2, g1); })) 
                return;
        ...
        if (g2.battle.status.hp > 0 && skill != Skill.dr) {
            ...
            if (g1 != g2 && d.swap(function() { return applySD(r, d, g2, g1); })) {
                ...
                if (!d.swap(function() { return applyCurse(r, d, g2, g1); }))
