...
function navCard(cid, bid) {
    ...
    if (!hasPage(pid)) {
        ...
        .append(
            ...
            .append(nSeparator("Max Card Stats (AceR)"))
        ...
        if (!hasPage(pid)) {
            ...
            for (var i = 0; i < cates.length; i++) {
                ...
                for (var j = 0; j < cate.cards.length; j++) {
                    var stats = cate.cards[j].getMaxStatus(buff);
                    var suffix = v == "total" ? stats.hp + stats.mp + stats.atk + stats.def + stats.agi + stats.wis : stats[v.toLowerCase()];
                    $("#" + lid)
        ...
        for (var i = 0; i < Skill.all.length; i++) {
            ...
            $("#" + lid)
                .append("<li>" + skill.name + " (" + skill.description + ")" +
                    "<p>MP Cost: " + (skill == Skill.ls ? "All" : skill.cost) + "</p>" +
                    "<p>HP Cost: " + skill.hp_cost + "</p>" +
                    "</li>");
        }
    }
    $.mobile.changePage("#" + pid);
...
function navNews() {
    ...
    if (!hasPage(pid)) {
        ...
        for (var date in whatisnew) {
            ...
            .append(nListview(lid));
            var cards = List.flatten(List.map(function(item) { return getCards(item); }, items));
            for (var i = 0; i < cards.length; i++) {
                var card = Card.get(cards[i]);
                $("#" + lid).append(mkCardItem(card, pid))
            }
            var excards = List.flatten(List.map(function(item) { return getExCards(item); }, items));
            for (var i = 0; i < excards.length; i++) {
                var card = ExCard.get(excards[i]);
                $("#" + lid).append(mkExCardItem(card, pid))
            }
        }
    }
    $.mobile.changePage("#" + pid);
