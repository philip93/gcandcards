...
function toString(target) {
    ...
    for (var i = 0; i < party.cards.length; i++) {
        ...
        strs.push("stoned=" + g.stone);
        var sks = g.getSkills();
        sks = List.map(function(s) { return s.id; }, sks);
        strs.push("skills=" + sks.join(","));
        if (g.stone == "custom") {
            strs.push("hp=" + g.status.hp);
            strs.push("mp=" + g.status.mp);
            strs.push("atk=" + g.status.atk);
            strs.push("def=" + g.status.def);
            strs.push("agi=" + g.status.agi);
            strs.push("wis=" + g.status.wis);
        }
        res.push(strs.join("&"));
    }
    if (party.ex1 != null) {
        res.push("ex1:id=" + party.ex1.id + "&skill=" + party.ex1.getSkill().id);
    }
    if (party.ex2 != null) {
        res.push("ex2:id=" + party.ex2.id + "&skill=" + party.ex2.getSkill().id);
    }
    return res.join(" +\n");
}
...
function fromLine(prefix, i, line) {
    ...
    if (ex1 || ex2) {
        ...
        if (c != null) {
            ...
            setExSkill(prefix, ex1 ? 1 : 2, skill);
        }
    } else {
        var id = -1;
        var type = 0;
        ...
        if (g != null) {
            ...
            setBorder(prefix, i, g.border.id);
            setCard(prefix, i, id);
...
function loadOptions() {
    ...
    options.dr_reset = parseInt($("#options-couter-reset").val());
}
    ...
    $("#option-dialog")
        .dialog({
            ...
            close: function(event, ui) {
                ...
                setCookie(COOKIE_ME_SD, options.me.sd);
                setCookie(COOKIE_ME_LS, options.me.ls);
                setCookie(COOKIE_ME_REVIVAL, options.me.revival);
                setCookie(COOKIE_ME_DS, options.me.ds);
                setCookie(COOKIE_ME_SAP, options.me.sap);
                setCookie(COOKIE_ME_GS, options.me.gs);
                setCookie(COOKIE_ME_MR, options.me.mr);
                setCookie(COOKIE_ME_MR2, options.me.mr2);
                setCookie(COOKIE_OPPO_SD, options.oppo.sd);
                setCookie(COOKIE_OPPO_LS, options.oppo.ls);
                setCookie(COOKIE_OPPO_REVIVAL, options.oppo.revival);
                setCookie(COOKIE_OPPO_DS, options.oppo.ds);
                setCookie(COOKIE_OPPO_SAP, options.oppo.sap);
                setCookie(COOKIE_OPPO_GS, options.oppo.gs);
                setCookie(COOKIE_OPPO_MR, options.oppo.mr);
                setCookie(COOKIE_OPPO_MR2, options.oppo.mr2);
